/**
 * Specs Inc. 2026
 * Enemy Instantiator component — spawns and animates EnemyCube obstacle prefabs toward a target.
 */
import { Logger } from "Utilities.lspkg/Scripts/Utils/Logger"
import { bindStartEvent, bindUpdateEvent } from "SnapDecorators.lspkg/decorators"

@component
export class EnemyInstantiator extends BaseScriptComponent {
  @ui.label('<span style="color: #4ADE80;">EnemyInstantiator – spawn and animate enemy obstacle cubes</span><br/><span style="color: #94A3B8; font-size: 11px;">Instantiates enemy prefabs at regular intervals and moves them toward a target. Cubes are destroyed on end-zone collision.</span>')
  @ui.separator
  ...
  @input
  @allowUndefined
  @hint("SceneObject with a box collider that destroys enemies on contact (e.g. InstantiatorEndPosition1)")
  ...
  onAwake(): void {
    this.logger = new Logger("EnemyInstantiator", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  onStart(): void {
    ...
    this.logger.info("Enemy Instantiator started")
  ...
  public reset(): void {
    ...
    this.nextSpawnTime = getTime() + this.spawnInterval
    this.logger.info("EnemyInstantiator reset")
  }
  ...
  private setupEndZone(): void {
    if (!this.endZone) {
      this.logger.warn("endZone is not assigned — enemies will not be destroyed at the end")
      return
    }
    ...
    ;(collider as any).onOverlapEnter.add((e: any) => {
      try {
        ...
        const match = objName.match(/EnemyCube_(\w+)/)
        if (!match) return
        this.pendingRemoval.add(match[1])
        try { obj.destroy() } catch (_) {}
        this.logger.info(`End zone destroyed enemy: ${objName}`)
      } catch (_) {}
    })
  ...
  spawnPrefab(): void {
    ...
    instance.name = `EnemyCube_${instanceId}`
    this.activePrefabs.push(instance)

    this.logger.info(`Spawned enemy at ${spawnPos.x.toFixed(2)}, ${spawnPos.y.toFixed(2)}, ${spawnPos.z.toFixed(2)}`)
  }
  ...
  updatePrefabs(): void {
    ...
    for (let i = 0; i < this.activePrefabs.length; i++) {
      ...
      try {
        ...
        const idMatch = prefab.name.match(/EnemyCube_(\w+)/)
