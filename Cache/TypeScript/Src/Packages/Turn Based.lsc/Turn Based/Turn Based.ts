...
export const NUMBER_USERS: number = 2;
...
export class TurnBased extends BaseScriptComponent {
    ...
    async getOtherUserIndex(): Promise<number> {
        return NUMBER_USERS - 1 - await this.getCurrentUserIndex();
    }
    ...
    async getUser(userIndex: number): Promise<SnapchatUser | null> {
        if (isNull(userIndex) || isNaN(userIndex) || userIndex < 0 || userIndex >= NUMBER_USERS) {
            return Promise.reject("User " + userIndex + " does not exist");
        }
        const turnCount = await this.turnDataController.getTurnCount();
        let isCurrentUser = turnCount % NUMBER_USERS === userIndex;
    ...
    async getOtherUser(): Promise<SnapchatUser> {
        return this.getUser(NUMBER_USERS - 1 - (await this.getCurrentUserIndex()));
    }
    ...
    private async getOtherUserDataStorage(): Promise<StorageAccessor> {
        return this.getUserDataStorage(NUMBER_USERS - 1 - await this.getCurrentUserIndex());
    }
