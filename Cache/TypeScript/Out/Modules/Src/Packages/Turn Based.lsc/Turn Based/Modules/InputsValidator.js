...
const TappablesValidator_1 = require("./Tappables/TappablesValidator");
class InputsValidator {
    constructor(logger) {
        this.logger = logger;
    }
    validateTurnLimit(useTurnLimit, turnLimit) {
        if (useTurnLimit) {
    ...
    validateTappableAreas(tappableAreas) {
    ...
    isTappableAreaValid(key, st) {
        ...
        if (key.length > TappablesValidator_1.TappablesValidator.KEY_LENGTH_LIMIT) {
            this.logger.logWarning("Warning: \"" + key + "\" key for tappable area must be " + TappablesValidator_1.TappablesValidator.KEY_LENGTH_LIMIT + " characters or fewer.");
            return false;
        }
        if (isNull(st)) {
    ...
    validateFlowSceneObjects(name, flowSceneObjectsSO) {
    ...
    isTurnVariableKeyValid(key) {
        return !isNull(key);
    }
    validateUserDefinedGameVariableInputs(name, inputs) {
        if (isNull(inputs)) {
            this.logger.logWarning(name + " can not be null or undefined. Skipping " + name);
            return [];
        }
    ...
    validateDebugTurnCount(turnCount) {
    ...
    validateDebugAssociatedDataString(json) {
        json = json.split("\\{")
            .join("{")
            .split("\\}")
            .join("}");
    ...
    validateDebugTurnHistoryStrings(debugTurnHistory) {
    ...
    validateDebugTurnHistoryStudioInputs(inputs) {
        if (isNull(inputs)) {
    ...
    validateTurnsSavedLimit(useTurnHistory, turnsSavedLimit) {
        if (useTurnHistory) {
