...
/**
 * Specs Inc. 2026
 * Runtime spline gizmo for visualizing smooth curves through control points. Renders Catmull-Rom
 * interpolated splines with customizable tension, interpolation density, closed loop support, and
 * real-time control point tracking for dynamic curve visualization.
 */
...
let Spline = (() => {
    ...
    var Spline = _classThis = class extends _classSuper {
        constructor() {
            super();
            this.controlPoints = (__runInitializers(this, _instanceExtraInitializers), this.controlPoints);
        ...
        onAwake() {
            // Initialize logger
            this.logger = new Logger_1.Logger("Spline", this.enableLogging || this.enableLoggingLifecycle, true);
            if (this.enableLoggingLifecycle) {
                this.logger.debug("LIFECYCLE: onAwake() - Component initializing");
            }
            if (!this.controlPoints || this.controlPoints.length < 2) {
                this.logger.debug("Error: At least 2 control points are required for a spline!");
                return;
            }
        ...
        createSpline() {
            if (this.splinePoints.length < 2) {
                this.logger.debug("Error: Not enough points to create a spline!");
                return;
            }
        ...
        setControlPoints(points) {
            if (points.length < 2) {
                this.logger.debug("Error: At least 2 control points are required!");
                return;
            }
        ...
        removeControlPoint(index) {
            ...
            if (this.controlPoints.length <= 2) {
                this.logger.debug("Error: Cannot remove point. At least 2 control points are required!");
                return;
            }
