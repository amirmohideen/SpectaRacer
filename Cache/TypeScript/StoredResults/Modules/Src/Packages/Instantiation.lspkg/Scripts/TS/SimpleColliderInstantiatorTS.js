...
exports.SimpleColliderInstantiatorTS = void 0;
var __selfType = requireType("./SimpleColliderInstantiatorTS");
...
/**
 * Specs Inc. 2026
 * Collider-based instantiation utility that spawns prefabs inside or on the surface of collider
 * shapes. Works with any collider geometry for flexible spatial distribution.
 */
let SimpleColliderInstantiatorTS = (() => {
    ...
    var SimpleColliderInstantiatorTS = _classThis = class extends _classSuper {
        constructor() {
            super();
            ...
            this.instantiateInside = this.instantiateInside;
            this.prefabScale = this.prefabScale;
            this.randomScaleVariation = this.randomScaleVariation;
        ...
        onAwake() {
            // Initialize logger
            this.logger = new Logger_1.Logger("SimpleColliderInstantiatorTS", this.enableLogging || this.enableLoggingLifecycle, true);
        ...
        instantiateInsideCollider() {
            if (!this.targetObject || !this.prefab) {
                this.logger.debug("Error: Target object or prefab not assigned!");
                return;
            }
        ...
        getShapeType() {
            ...
            try {
                ...
                this.logger.debug("DEBUG: Using component type check fallback");
                ...
                for (let i = 0; i < components.length; i++) {
                    ...
                    this.logger.debug("DEBUG: Found component type: " + typeName);
                    if (typeName.includes("Box"))
                        return "box";
                ...
                this.logger.debug("Error detecting shape type: " + e);
            }
            this.logger.debug("DEBUG: No specific shape detected, using box as default");
            return shapeType;
        }
        ...
        getRandomPointOnColliderSurface() {
            const shapeType = this.getShapeType();
            switch (shapeType) {
                case "box":
        ...
        randomPointInBox() {
            ...
            this.logger.debug("DEBUG: Using object scale as box size: " +
                boxSize.x.toFixed(2) + ", " +
                boxSize.y.toFixed(2) + ", " +
                boxSize.z.toFixed(2));
            // Generate random point within box extents (in local space)
            const localPoint = new vec3((Math.random() - 0.5) * boxSize.x, (Math.random() - 0.5) * boxSize.y, (Math.random() - 0.5) * boxSize.z);
            // Transform to world space
            return this.transformPoint(localPoint, centerPos, rotation);
        }
        randomPointOnBoxSurface() {
            const transform = this.targetObject.getTransform();
            const centerPos = transform.getWorldPosition();
            const rotation = transform.getWorldRotation();
            // Get box size from the target object's scale
            const objectScale = transform.getWorldScale();
            const boxSize = new vec3(objectScale.x, objectScale.y, objectScale.z);
            this.logger.debug("DEBUG: Using object scale as box size for surface: " +
                boxSize.x.toFixed(2) + ", " +
                boxSize.y.toFixed(2) + ", " +
                boxSize.z.toFixed(2));
            // Half extents
            ...
            switch (face) {
                case 0: // +X
                    localPoint = new vec3(halfWidth, (Math.random() - 0.5) * boxSize.y, (Math.random() - 0.5) * boxSize.z);
                    break;
                case 1: // -X
                    localPoint = new vec3(-halfWidth, (Math.random() - 0.5) * boxSize.y, (Math.random() - 0.5) * boxSize.z);
                    break;
                case 2: // +Y
                    localPoint = new vec3((Math.random() - 0.5) * boxSize.x, halfHeight, (Math.random() - 0.5) * boxSize.z);
                    break;
                case 3: // -Y
                    localPoint = new vec3((Math.random() - 0.5) * boxSize.x, -halfHeight, (Math.random() - 0.5) * boxSize.z);
                    break;
                case 4: // +Z
                    localPoint = new vec3((Math.random() - 0.5) * boxSize.x, (Math.random() - 0.5) * boxSize.y, halfDepth);
                    break;
                case 5: // -Z
                    localPoint = new vec3((Math.random() - 0.5) * boxSize.x, (Math.random() - 0.5) * boxSize.y, -halfDepth);
                    break;
        ...
        randomPointInSphere() {
            ...
            this.logger.debug("DEBUG: Using sphere radius: " + radius.toFixed(2));
            ...
            const randomPoint = new vec3(x * normalizedDistance, y * normalizedDistance, z * normalizedDistance);
            this.logger.debug(`DEBUG: Generated sphere point at local coordinates (${randomPoint.x.toFixed(2)}, ${randomPoint.y.toFixed(2)}, ${randomPoint.z.toFixed(2)})`);
            // Transform to world space (apply rotation and translation)
            return this.transformPoint(randomPoint, centerPos, rotation);
        }
        randomPointOnSphereSurface() {
            const transform = this.targetObject.getTransform();
            const centerPos = transform.getWorldPosition();
            const rotation = transform.getWorldRotation();
            // Try to find the sphere radius from the collider component
            ...
            this.logger.debug("DEBUG: Using sphere surface radius: " + radius.toFixed(2));
            ...
            const randomPoint = new vec3(x * normalizedDistance, y * normalizedDistance, z * normalizedDistance);
            this.logger.debug(`DEBUG: Generated sphere surface point at local coordinates (${randomPoint.x.toFixed(2)}, ${randomPoint.y.toFixed(2)}, ${randomPoint.z.toFixed(2)})`);
            // Transform to world space (apply rotation and translation)
            return this.transformPoint(randomPoint, centerPos, rotation);
        }
            ...
            try {
                ...
                for (let i = 0; i < components.length; i++) {
                    ...
                    if (typeName.includes("Sphere")) {
                        ...
                        try {
                            const sphereComponent = component;
                            if (sphereComponent.radius !== undefined) {
                                // Found radius property!
                                this.logger.debug("DEBUG: Found sphere collider with radius: " + sphereComponent.radius);
                                return sphereComponent.radius;
                            }
                        }
                        catch (e) {
                            this.logger.debug("Error getting radius from sphere component: " + e);
                        }
                    }
                }
            }
            catch (e) {
                this.logger.debug("Error searching for sphere collider: " + e);
            }
            // No sphere collider found or couldn't get radius
            return 0;
        }
        // Fallback method to derive sphere radius from transform
        getSphereRadiusFromTransform() {
            const transform = this.targetObject.getTransform();
            const objectScale = transform.getWorldScale();
            // Use the smallest dimension to ensure sphere fits within object bounds
            const radius = Math.min(objectScale.x, objectScale.y, objectScale.z) / 2.0;
            this.logger.debug("DEBUG: Derived sphere radius from transform scale: " + radius.toFixed(2));
            return radius;
        }
            ...
            try {
                ...
                for (let i = 0; i < components.length; i++) {
                    ...
                    if (typeName.includes("Capsule")) {
                        ...
                        try {
                            const capsuleComponent = component;
                            ...
                            if (radius > 0) {
                                this.logger.debug(`DEBUG: Found capsule collider with radius: ${radius}, length: ${length}, axis: ${axis}`);
                                return { valid: true, radius, length, axis };
                            }
                        }
                        catch (e) {
                            this.logger.debug("Error getting properties from capsule component: " + e);
                        }
                    }
                }
            }
            catch (e) {
                this.logger.debug("Error searching for capsule collider: " + e);
            }
        ...
        getCapsulePropertiesFromTransform() {
            const transform = this.targetObject.getTransform();
            const objectScale = transform.getWorldScale();
            ...
            this.logger.debug(`DEBUG: Derived capsule properties from transform - radius: ${radius.toFixed(2)}, length: ${length.toFixed(2)}, axis: ${axis}`);
            return { valid: true, radius, length, axis };
        }
        // Helper method to transform a point from local to world space
        transformPoint(localPoint, position, rotation) {
            // Apply rotation
            const rotatedPoint = this.rotatePointByQuaternion(localPoint, rotation);
            // Apply translation
            const worldPoint = new vec3(position.x + rotatedPoint.x, position.y + rotatedPoint.y, position.z + rotatedPoint.z);
            // Print transformation info
            this.logger.debug(`DEBUG: Local point ${localPoint.x.toFixed(2)}, ${localPoint.y.toFixed(2)}, ${localPoint.z.toFixed(2)} 
        ...
        rotatePointByQuaternion(point, rotation) {
        ...
        createPrefabInstance(position) {
            if (this.prefab) {
                ...
                this.logger.debug(`Created prefab instance at position: ${instancePosition.x.toFixed(2)}, ${instancePosition.y.toFixed(2)}, ${instancePosition.z.toFixed(2)} with scale: ${instance.getTransform().getLocalScale().x.toFixed(2)}`);
            }
        }
    };
    __setFunctionName(_classThis, "SimpleColliderInstantiatorTS");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        _onStart_decorators = [decorators_1.bindStartEvent];
        __esDecorate(_classThis, null, _onStart_decorators, { kind: "method", name: "onStart", static: false, private: false, access: { has: obj => "onStart" in obj, get: obj => obj.onStart }, metadata: _metadata }, null, _instanceExtraInitializers);
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        SimpleColliderInstantiatorTS = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return SimpleColliderInstantiatorTS = _classThis;
})();
exports.SimpleColliderInstantiatorTS = SimpleColliderInstantiatorTS;
//# sourceMappingURL=SimpleColliderInstantiatorTS.js.map