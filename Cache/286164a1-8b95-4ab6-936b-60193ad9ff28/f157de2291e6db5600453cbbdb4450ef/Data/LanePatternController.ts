...
export class LanePatternController extends BaseScriptComponent {
  ...
  @input
  @hint("Seconds each pattern wave stays active. Should be long enough for each instantiator to fire at least once based on its spawnInterval.")
  waveDuration: number = 4.0
  ...
  onStart(): void {
    ...
    this.logger.info("LanePatternController started — initial gap before wave 0")
  }
  ...
  private applyPattern(pattern: number[]): void {
    for (let i = 0; i < 3; i++) {
      ...
      if (this.coins[i]) {
        ...
        if (wantCoin) this.coins[i].resetSpawnTimer()
      }
      if (this.trees[i]) {
        this.trees[i].spawnEnabled = wantTree
        if (wantTree) this.trees[i].resetSpawnTimer()
      }
    }
  }
