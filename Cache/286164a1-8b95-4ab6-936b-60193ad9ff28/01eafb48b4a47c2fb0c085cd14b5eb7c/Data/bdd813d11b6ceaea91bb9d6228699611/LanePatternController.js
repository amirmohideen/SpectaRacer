...
let LanePatternController = (() => {
    ...
    var LanePatternController = _classThis = class extends _classSuper {
        ...
        // ─── Pause / resume (called by PlayPauseManager) ───────────────────────────
        pauseAll() {
            this.isPaused = true;
            for (let i = 0; i < 3; i++) {
                if (this.coins[i])
            ...
            this.logger.info("LanePatternController: all lanes paused and frozen");
        }
        resumeAll() {
            this.isPaused = false;
            for (let i = 0; i < 3; i++) {
                ...
                // coins/trees spawnEnabled is managed by the state machine; it will restore on next cycle
            }
            // updateMoveSpeed() runs on the very next onUpdate frame and restores the correct ramped speed
            this.logger.info("LanePatternController: all lanes resumed");
        }
