...
var MAT_RENDER;
var VFX_OBJECT;
var INSTANCE_COUNT = 7000;
var TEXEL_COUNT = 4;
var DIMENSIONS = new vec2(2048.0, 14.0);
var COPY_ID = 0;
var MAX_COPIES = 32
...
VfxManager.prototype.updateEnable = function(pass) {
	...
	for (const vfxObj of pass.vfxObjects) {
		...
		for (var i = 0; i < vfxObj.sceneObjects.length; i++) {
			if (isNull(vfxObj.sceneObjects[i]) || isNull(vfxObj.vfxComponents[i])) continue;
...
var updateUniformsLocal = function(id, scriptTransform, manager)
{
	...
	// const modelMatrixInv = this.usingScreenTransform[id] ? modelMatrix.inverse() : scriptTransform.getInvertedWorldTransform();
	...
	*/
	...
	// Guard against mid-update destruction
	if (isNull(vfxComponent) || isNull(renderMeshVisual) || isNull(renderObject) || isNull(vfxSceneObject)) {
		return;
	}
	...
	/*  The following uniforms are derived from vfxViewMatrixInverse
