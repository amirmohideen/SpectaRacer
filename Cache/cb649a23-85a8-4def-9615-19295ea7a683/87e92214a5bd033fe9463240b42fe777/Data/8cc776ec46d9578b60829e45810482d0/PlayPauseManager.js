...
let PlayPauseManager = (() => {
    ...
    var PlayPauseManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.laneSpawnManager = this.laneSpawnManager;
            ...
            this.pauseInteractable = this.pauseInteractable;
            this.playInteractable = this.playInteractable;
            this.restartInteractable = this.restartInteractable;
        ...
        pause() {
            if (this.isPaused)
                return;
            this.isPaused = true;
            // Freeze both spawning systems
            if (this.laneController)
                this.laneController.isPaused = true;
            if (this.laneSpawnManager)
                this.laneSpawnManager.isPaused = true;
            // Zero out individual instantiator speeds so in-flight tiles stop moving
            for (let i = 0; i < 3; i++) {
                if (this.coins[i])
        ...
        resume() {
            if (!this.isPaused)
                return;
            this.isPaused = false;
            // Re-enable road spawning; LPC will restore speeds on its next update
            for (let i = 0; i < 3; i++) {
                if (this.roads[i])
