/**
 * Specs Inc. 2026
 * Coin Instantiator component — spawns and animates CoinCube prefabs toward a target.
 */
import { Logger } from "Utilities.lspkg/Scripts/Utils/Logger"
import { bindStartEvent, bindUpdateEvent } from "SnapDecorators.lspkg/decorators"

@component
export class CoinInstantiator extends BaseScriptComponent {
  @ui.label('<span style="color: #FBBF24;">CoinInstantiator – spawn and animate coin target cubes</span><br/><span style="color: #94A3B8; font-size: 11px;">Instantiates coin prefabs at regular intervals and moves them toward a target. Cubes are destroyed on end-zone collision.</span>')
  @ui.separator
  ...
  @input
  @allowUndefined
  @hint("SceneObject with a box collider that destroys coins on contact (e.g. InstantiatorEndPosition1)")
  ...
  onAwake(): void {
    this.logger = new Logger("CoinInstantiator", this.enableLogging || this.enableLoggingLifecycle, true)
  ...
  onStart(): void {
    ...
    this.logger.info("Coin Instantiator started")
  ...
  private setupEndZone(): void {
    if (!this.endZone) {
      this.logger.warn("endZone is not assigned — coins will not be destroyed at the end")
      return
    }
    ...
    ;(collider as any).onOverlapEnter.add((e: any) => {
      try {
        ...
        const match = objName.match(/CoinCube_(\w+)/)
        if (!match) return
        this.pendingRemoval.add(match[1])
        try { obj.destroy() } catch (_) {}
        this.logger.info(`End zone destroyed coin: ${objName}`)
      } catch (_) {}
    })
  ...
  private drawGizmo(): void {
    ...
    debugRender.drawLine(spawnPos, targetPos, new vec4(1, 0.84, 0, 1))
    debugRender.drawSolidSphere(spawnPos, 0.03, new vec4(1, 0.84, 0, 0.8))
    debugRender.drawSolidSphere(targetPos, 0.03, new vec4(1, 0.4, 0, 0.8))
  }
  ...
  spawnPrefab(): void {
    ...
    instance.name = `CoinCube_${instanceId}`
    this.activePrefabs.push(instance)

    this.logger.info(`Spawned coin at ${spawnPos.x.toFixed(2)}, ${spawnPos.y.toFixed(2)}, ${spawnPos.z.toFixed(2)}`)
  }
  ...
  updatePrefabs(): void {
    ...
    for (let i = 0; i < this.activePrefabs.length; i++) {
      ...
      try {
        ...
        const idMatch = prefab.name.match(/CoinCube_(\w+)/)
