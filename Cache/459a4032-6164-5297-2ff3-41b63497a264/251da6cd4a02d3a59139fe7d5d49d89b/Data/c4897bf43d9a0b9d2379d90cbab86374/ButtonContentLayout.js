...
exports.ButtonContentLayout = exports.ButtonContentLayoutConfig = void 0;
/**
 * Specs Inc. 2026
 ...
 */
class ButtonContentLayoutConfig {
    constructor() {
        // Image settings
        this.showImage = true;
        this.imageSize = 2;
        ...
        // Layout settings
        this.imageToTitleSpacing = 0.4; // Spacing between image and title
        this.titleToSubtitleSpacing = 0.1; // Spacing between title and subtitle (tighter)
        this.offsetX = 0; // Container X offset from button center
        this.offsetY = 0; // Container Y offset from button center
        this.imageOffsetX = 0; // Additional X offset for image only (relative to container)
        this.buttonWidth = 10; // Button width for alignment calculations
        ...
        this.textLayoutRectLeft = -7.5;
        this.textLayoutRectRight = 7.5;
        this.textLayoutRectBottom = -2.25;
        this.textLayoutRectTop = 2.25;
    }
}
exports.ButtonContentLayoutConfig = ButtonContentLayoutConfig;
class ButtonContentLayout {
    constructor(buttonObject, config) {
    ...
    create() {
        ...
        this.containerObject.getTransform().setLocalPosition(new vec3(this.config.offsetX, this.config.offsetY, 0.15));
    ...
    createImage(yPosition) {
        ...
        imageObj.getTransform().setLocalPosition(new vec3(this.config.imageOffsetX, yPosition, 0));
        imageObj.getTransform().setLocalScale(new vec3(this.config.imageSize, this.config.imageSize, 1));
        return imageObj;
    }
    /**
     * Create title text component (parented to textContainer)
     */
    createTitle(yPosition) {
        ...
        const screenTransform = titleObj.createComponent("Component.ScreenTransform");
        ...
        const titleText = titleObj.createComponent("Component.Text");
    ...
    createSubtitle(yPosition) {
        ...
        screenTransform.anchors = anchors;
        const subtitleText = subtitleObj.createComponent("Component.Text");
        ...
        subtitleObj.getTransform().setLocalPosition(new vec3(0, 0, 0));
        return subtitleObj;
    }
    /**
     * Update text content
    ...
    updateText(title, subtitle) {
        if (title && this.titleObject) {
            const titleText = this.titleObject.getComponent("Component.Text");
            if (titleText) {
                titleText.text = title;
            }
        }
        if (subtitle && this.subtitleObject) {
            const subtitleText = this.subtitleObject.getComponent("Component.Text");
            if (subtitleText) {
     ...
     * Update colors
     */
    updateColors(titleColor, subtitleColor) {
     ...
     * Get created objects
     */
    getContainerObject() { return this.containerObject; }
    getImageObject() { return this.imageObject; }
    getTitleObject() { return this.titleObject; }
    getSubtitleObject() { return this.subtitleObject; }
    /**
     * Get image component for material assignment
     */
    getImageComponent() {
        if (this.imageObject) {
            return this.imageObject.getComponent("Component.Image");
        }
        return null;
    }
    /**
...
exports.ButtonContentLayout = ButtonContentLayout;
//# sourceMappingURL=ButtonContentLayout.js.map